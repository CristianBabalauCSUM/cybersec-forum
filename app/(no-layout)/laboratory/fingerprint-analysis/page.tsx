...
export default function DeviceSecurityAnalysisPage() {
  ...
  // Enhanced suspicious detection rules
  const suspiciousRules: SuspiciousRule[] = [
    ...
    // Screen Rules
    {
      ...
      parameter: 'screen',
      condition: (_, fp) => {
        const common = [[1024, 768], [1280, 1024], [1366, 768], [1920, 1080], [800, 600]];
    ...
    },
    {
      ...
      condition: (value) => /Software|Emulation|Virtual|VM|ANGLE|SwiftShader|llvmpipe/i.test(value),
      severity: 'high',
    ...
    },
    {
      ...
      condition: (value) => value === 'unavailable' || value === 'suspended',
      severity: 'medium',
      description: 'Audio context in problematic state',
      points: 15
    },
    {
      category: 'Audio',
  ...
  const handleGenerateFingerprint = async () => {
    ...
    try {
      console.log('=== STARTING DEVICE SECURITY ANALYSIS ===');
      console.log('Timestamp:', new Date().toISOString());
      
      const fp = await generateFingerprint();
      console.log('=== COMPLETE FINGERPRINT DATA ===');
      ...
      console.log('Risk Factors:', fp.riskFactors);
  ...
  const analyzeSuspiciousItems = (fp: DeviceFingerprint) => {
    console.log('=== STARTING SUSPICIOUS ITEMS ANALYSIS ===');
    
    const detected: Array<SuspiciousRule & { value: any }> = [];
    let totalPoints = 0;

    suspiciousRules.forEach((rule, index) => {
      try {
        let value: any;
        ...
        console.log(`Rule ${index + 1}: ${rule.category} - ${rule.parameter}`, {
          value,
        ...
        if (rule.condition(value, fp)) {
          ...
          console.log(`🚨 SUSPICIOUS ITEM DETECTED:`, {
        ...
        console.warn(`Error evaluating rule ${index + 1}:`, ruleError);
      }
    });

    console.log('=== SUSPICIOUS ITEMS ANALYSIS COMPLETE ===');
    console.log(`Total suspicious items: ${detected.length}`);
    console.log(`Total risk points: ${totalPoints}`);
  ...
  if (!fingerprint) {
    console.log('=== RENDERING NO FINGERPRINT STATE ===');
    return (
      <div className="max-w-6xl mx-auto p-6">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
          <h2 className="text-xl font-semibold text-red-800 mb-2">Analysis Failed</h2>
  ...
  console.log('=== RENDERING MAIN ANALYSIS DASHBOARD ===');
